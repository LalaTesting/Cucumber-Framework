 ...
 * Description: This test script verifies the clicking on playlist name from playlist (first) from 
...
public class VerifyNavigatingToWatchPageClickingOnPlaylistName extends BaseTest {
	...
	/**
	 * Click on Playlist name from playlist
	 */
	@Test(description = "Step 6: Click on Playlist name from playlist.", priority = 6)
	public void Step06_ClickOnPlaylistName() throws Exception {

		String playlistName = driver.findElement(
				By.xpath(ObjRepoProp
						.getProperty("playlistsPageFirstPlaylistName_XPATH")))
				.getText();

		// Click on Playlist name
		watchPage = playlistPage.clickOnFirstPlaylistName();

		log.info("Clicked Playlist Name: " + playlistName + ".\n");
		Reporter.log("<p>Clicked Playlist Name: " + playlistName);
