 ...
 * Description: This test script verifies navigating to browse shows from my shows page when there are followed 
 * shows by clicking on show name from video section.
 * Created on 15-November-2016 
 * Last updated on 15-November-2016
 * 
 */

public class VerifyNavigatingToShowsPageFromMyshowsPageWhenFollowedShowsFromVideo
		extends BaseTest {
	...
	public void Step06_VerifyVideosPresentUnderShowsNameTitle()
			throws Exception {

		// Verify fvideos are present under followed shows name
		int VideoCount = driver.findElements(
				By.xpath(ObjRepoProp
						.getProperty("myShowsPageFollowedVideoImg_XPATH")))
				.size();
		if (VideoCount > 0) {
			videoPresent = true;
	...
	public void Step07_ClickOnShowNameTitleFromVideo() throws Exception {

		if (videoPresent == true) {

			// Getting the show title from from first row
			String firstShowHeadingTitle = driver
					.findElement(
							By.xpath(ObjRepoProp
			...
			showsPage = myShowsPage.clickOnFirstVideoShowTitle();
