	...
	public static void loadProprtyFile()
	{
		...
		try {
			...
			if (rootDir.contains("WatchableWebClientNewUI"))
			{
				...
				ObjRepoProp.load(new FileInputStream(rootDir+ "/resources/ObjRepository.properties"));

				TextProp.load(new FileInputStream(rootDir+ "/resources/Text.properties"));

			}

			else {
				
				ObjRepoProp.load(new FileInputStream(rootDir+ "/WatchableWebClientNewUI/resources/ObjRepository.properties"));
				TextProp.load(new FileInputStream(rootDir+ "/WatchableWebClientNewUI/resources/Text.properties"));
