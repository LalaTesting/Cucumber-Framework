 ...
 * Description: This test script verifies functionality of close button present in show email sharing popup window.
 * Created on 07-December-2016 
 * Last updated on 07-December-2016
 * 
 */

public class VerifyShowEmailSharingPopupCloseButtonFunctionality extends
		BaseTest {
	...
	public void Step10_ClickOnCloseButton() throws Exception {
		...
		// Click on close button
		watchPage.clickOnCloseButtonFromPlaylistSharingPopup();

		// Verifying Show email sharing popup window closed.
		Assert.assertFalse(
				driver.findElement(
						By.xpath(ObjRepoProp
								.getProperty("watchPageEmailPlaylistPopUp_XPATH")))
						.isDisplayed(),
				"Show email sharing popup window is not closed.");

		log.info("Show email sharing popup window is closed.\n");
		Reporter.log("<p>Show email sharing popup window is closed.");
