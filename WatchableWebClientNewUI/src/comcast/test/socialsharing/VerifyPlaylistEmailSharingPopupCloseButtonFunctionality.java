...
public class VerifyPlaylistEmailSharingPopupCloseButtonFunctionality extends
		BaseTest {
	...
	public void Step10_ClickOnCloseButton() throws Exception {
		...
		// Verifying Playlist email sharing popup window closed.
		Assert.assertFalse(
				driver.findElement(
						By.xpath(ObjRepoProp
								.getProperty("watchPageEmailPlaylistPopUp_XPATH")))
						.isDisplayed(),
				"Playlist email sharing popup window is not closed.");

		log.info("Playlist email sharing popup window is closed.\n");
		Reporter.log("<p>Playlist email sharing popup window is closed.");
